...
ConsoleCommands::ConsoleCommands(IConsole &console,
    ...
    : syncGuard(syncGuard), logger(logger, "[CONSOLE]"), console(console),
      environment(environment), ueRelay(ueRelay) {}
...
void ConsoleCommands::start() {
  ...
  console.addCommand("a", "Show address",
                     std::bind(&ConsoleCommands::showAddress, this,
                               argsArgument, streamArgument));
  console.addCommand("s", "Show status",
                     std::bind(&ConsoleCommands::showStatus, this, argsArgument,
                               streamArgument));
  console.addCommand("l", "List attached ue",
                     std::bind(&ConsoleCommands::listAttachedUe, this,
                               argsArgument, streamArgument));
  console.addCloseCommand();
  console.addHelpCommand();
  console.addCommand("t", "Test commands - details in implementation",
                     std::bind(&ConsoleCommands::testCommands, this,
                               argsArgument, streamArgument));
}
...
void ConsoleCommands::testCommands(std::string args, std::ostream &os) {
  ...
  try {
    ...
    parameters.sendMessage = [this](BinaryMessage message, PhoneNumber to) {
      SyncLock lock(*syncGuard);
