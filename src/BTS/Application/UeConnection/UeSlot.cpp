...
UeSlot::UeSlot(IImplPtr impl) : impl(impl) {}

bool UeSlot::sendMessage(BinaryMessage message, PhoneNumber to) {
  return impl->sendMessage(std::move(message), to);
}

void UeSlot::attach(PhoneNumber phone) { impl = impl->attach(phone); }

bool UeSlot::isAttached() const { return impl->isAttached(); }

PhoneNumber UeSlot::getPhoneNumber() const { return impl->getPhoneNumber(); }

void UeSlot::remove() { impl->remove(); }
...
bool UeSlot::NullImpl::isAttached() const { return false; }

PhoneNumber UeSlot::NullImpl::getPhoneNumber() const { return {}; }
