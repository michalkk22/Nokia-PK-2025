...
UeConnection::UeConnection(ITransportPtr transport, common::ILogger &logger,
                           SyncGuardPtr syncGuard)
    : syncGuard(syncGuard), transport(transport),
      logger(logger, std::bind(&UeConnection::printPrefix, this, _1)) {}

UeConnection::~UeConnection() { stop(); }

void UeConnection::start(UeSlot ueSlot) {
  this->ueSlot = ueSlot;
  transport->registerDisconnectedCallback(
      std::bind(&UeConnection::onUeDisconnectedCallback, this));
  transport->registerMessageCallback(
      std::bind(&UeConnection::onUeMessageCallback, this, _1));
}
...
void UeConnection::sendAttachResponse(bool success, PhoneNumber phoneNumber) {
  common::OutgoingMessage messageBuilder(MessageId::AttachResponse,
...
void UeConnection::sendSib(BtsId btsId) {
  common::OutgoingMessage messageBuilder(MessageId::Sib, PhoneNumber{},
                                         PhoneNumber{});
...
void UeConnection::sendUnknownRecipient(const MessageHeader &messageHeader) {
  common::OutgoingMessage messageBuilder(MessageId::UnknownRecipient,
...
void UeConnection::sendUnknownSender(const MessageHeader &messageHeader) {
  common::OutgoingMessage messageBuilder(MessageId::UnknownSender,
                                         PhoneNumber{}, getPhoneNumber());
  messageBuilder.writeMessageHeader(messageHeader);
  sendMessage(messageBuilder.getMessage());
}
...
bool UeConnection::isAttached() const { return ueSlot.isAttached(); }
...
void UeConnection::onAttachRequest(PhoneNumber phoneNumber) {
  ...
  if (isAttached()) {
    if (this->getPhoneNumber() == phoneNumber) {
      // special case #2
      logger.logError(
          "Attach to UE already attached with identical number accepted");
...
void UeConnection::print(std::ostream &os) const {
  os << transport->addressToString() << ":" << getPhoneNumber() << ":"
     << (isAttached() ? "A" : "I");
}
