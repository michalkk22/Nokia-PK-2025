...
UeConnectionSpawner::UeConnectionSpawner(
    ...
    std::shared_ptr<IUeRelay> ueRelay, SyncGuardPtr syncGuard)
    : environment(environment), ueConnectionFactory(ueConnectionFactory),
      ueRelay(ueRelay), logger(environment.getLogger(), "[SPAWNER]"),
      btsId(environment.getBtsId()), syncGuard(syncGuard) {}
...
void UeConnectionSpawner::start() {
  ...
  environment.registerUeConnectedCallback(std::bind(
      &UeConnectionSpawner::spawnConnection, this, std::placeholders::_1));
}
