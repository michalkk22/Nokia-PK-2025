...
UeConnectionSpawnerTestSuite::UeConnectionSpawnerTestSuite() {
  ...
  ueConnectionFactoryMock =
      std::make_shared<StrictMock<IUeConnectionFactoryMock>>();
  objectUnderTest = std::make_unique<UeConnectionSpawner>(
      environmentMock, ueConnectionFactoryMock, ueRelayMock, syncGuard);
}

void UeConnectionSpawnerTestSuite::setUpEnvironmentMock() {
  EXPECT_CALL(environmentMock, getLogger())
      .WillRepeatedly(ReturnRef(loggerMock));
...
void UeConnectionSpawnerTestSuite::expectRegisterCallback() {
  EXPECT_CALL(environmentMock, registerUeConnectedCallback(_))
      .WillOnce(SaveArg<0>(&ueConnectedCallback));
}
...
UeConnectionStartedSpawnerTestSuite::UeConnectionStartedSpawnerTestSuite() {
  ...
  transportMock =
      std::make_shared<testing::StrictMock<common::ITransportMock>>();
...
void UeConnectionStartedSpawnerTestSuite::expectUeCreated() {
  ...
  EXPECT_CALL(*ueConnectionFactoryMock, createConnection(expectedTransport))
      .WillOnce([this](auto &&) { return std::move(ueConnectionMockPtr); });
}
...
void UeConnectionStartedSpawnerTestSuite::expectUeAddedToRelay() {
  ...
  EXPECT_CALL(*ueRelayMock, add(expectedConnection))
      .WillOnce([this](auto p_arg) {
...
TEST_F(UeConnectionStartedSpawnerTestSuite,
       shallSpawnConnectionOnNewTransport) {
  expectUeCreated();
