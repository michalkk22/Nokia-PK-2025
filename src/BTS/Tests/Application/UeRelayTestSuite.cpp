...
void UeRelayTestSuite::shallForwardMessage(ConnectionMock &connnection) {
  ...
  ASSERT_TRUE(
      connnection.connectionSlot.sendMessage(MESSAGE, connnection.phoneNumber));
}

void UeRelayTestSuite::shallNotForwardMessage(PhoneNumber phoneNumber) {
  ASSERT_FALSE(
      connectionAttached.connectionSlot.sendMessage(MESSAGE, phoneNumber));
}

void UeRelayTestSuite::expectAction(
    UeRelayTestSuite::ConnectionMock &connnection) {
...
UeRelayTestSuite::ConnectionMock::ConnectionMock()
  ...
  EXPECT_CALL(*connectionMock, print(_))
      .WillRepeatedly(
          Invoke(this, &UeRelayTestSuite::ConnectionMock::printConnection));
}
...
void UeRelayTestSuite::ConnectionMock::remove() { connectionSlot.remove(); }

void UeRelayTestSuite::ConnectionMock::expectSendMessage(
...
TEST_F(UeRelayTestSuite, shallCountConnections) {
  ASSERT_EQ(connectionAdded.count() + connectionAttached.count() +
                connectionReAttached.count(),
