...
ConsoleCommandsTestSuite::ConsoleCommandsTestSuite() {
  ...
  objectUnderTest = std::make_unique<ConsoleCommands>(
      consoleMock, environmentMock, loggerMock, ueRelayMock, syncGuard);
}

void ConsoleCommandsTestSuite::expectRegisterCallback(
    IConsoleMock &consoleMock, std::string command,
    IConsole::CommandCallback &consoleCallback) {
  EXPECT_CALL(consoleMock, addCommand(command, _, _))
      .WillOnce(SaveArg<2>(&consoleCallback));
}
...
void ConsoleCommandsAfterStartTestSuite::expectCountNotAttached() {
  EXPECT_CALL(*ueRelayMock, countNotAttached())
      .WillOnce(Return(COUNT_NOT_ATTACHED));
}

void ConsoleCommandsAfterStartTestSuite::
...
void ConsoleCommandsAfterStartTestSuite::onCallback(
    IConsole::CommandCallback &callback, std::string args) {
...
void ConsoleCommandsAfterStartTestSuite::expectAttachedPrinted() {
  for (std::size_t i = 0u; i < COUNT_ATTACHED_TO_VISIT; ++i) {
    EXPECT_CALL(ueConnectionAttachedMock[i], print(_))
        .WillOnce(Invoke(
            [&, i](auto &os) { os << ueConnectionAttachedPrintout[i]; }));
  }
  EXPECT_CALL(*ueRelayMock, visitAttachedUe(_))
      .WillOnce(
          Invoke(this, &ConsoleCommandsAfterStartTestSuite::applyUeAttached));
}
...
void ConsoleCommandsAfterStartTestSuite::applyUeAttached(
