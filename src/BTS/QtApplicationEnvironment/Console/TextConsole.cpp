...
void TextConsole::addCommand(std::string command,
                             const std::string &commandText,
                             IConsole::CommandCallback commandCallback) {
  logger.logDebug("Add: `", command, "` for \"", commandText, "\"");
  commands.push_back({command, commandText, commandCallback});
}

void TextConsole::addCloseCommand(std::string command,
                                  const std::string &commandText,
                                  IConsole::CommandCallback commandCallback) {
...
void TextConsole::addHelpCommand(std::string command,
                                 const std::string &commandText) {
  logger.logDebug("Add help: `", command, "` for \"", commandText, "\"");
  auto helpCommand = [this](std::string, std::ostream &os) { printHelp(os); };
...
IConsole::CommandCallback
TextConsole::getCallback(std::string commandText) const {
  auto commandMatch = [commandText](const Command &command) {
    return commandText == command.command;
  };
