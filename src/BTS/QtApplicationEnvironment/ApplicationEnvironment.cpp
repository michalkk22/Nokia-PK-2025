...
ApplicationEnvironment::ApplicationEnvironment(int &argc, char *argv[])
    : configuration(ApplicationEnvironment::readConfiguration(argc, argv)),
      ...
      logFile(logFilename(btsId)), logger(logFile), qApplication(argc, argv),
      console(logger), transportEnvironment(logger, *configuration) {
  QObject::connect(&console, SIGNAL(quit()), &qApplication, SLOT(quit()));
}

IConsole &ApplicationEnvironment::getConsole() { return console; }

void ApplicationEnvironment::registerUeConnectedCallback(
    UeConnectedCallback newCallback) {
  transportEnvironment.registerUeConnectedCallback(newCallback);
}

ILogger &ApplicationEnvironment::getLogger() { return logger; }

BtsId ApplicationEnvironment::getBtsId() const { return btsId; }
...
std::unique_ptr<common::MultiLineConfig>
ApplicationEnvironment::readConfiguration(int argc, char *argv[]) {
  auto commandLineConfig =
      std::make_unique<common::MultiLineConfig>(argc - 1, argv + 1);
