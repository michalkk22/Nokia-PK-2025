...
QtTransportEnvironment::QtTransportEnvironment(common::ILogger &logger,
                                               common::MultiLineConfig &config)
    : logger(logger), port(config.getNumber<decltype(port)>("port", 8181)) {}
...
void QtTransportEnvironment::exec() {
  ...
  if (manager.capabilities() &
    ...
    QObject::connect(session.get(), &QNetworkSession::opened,
                     std::bind(&QtTransportEnvironment::sessionOpened, this));
    session->open();
  } else {
    sessionOpened();
  }
  QObject::connect(
      ...
      std::bind(&QtTransportEnvironment::handleNewConnection, this));
}
...
void bts::QtTransportEnvironment::registerUeConnectedCallback(
...
void QtTransportEnvironment::handleNewConnection() {
  ...
  if (socket) {
    ...
    if (ueConnectedCallback) {
      ...
      logger.logError("New connection from: ", ueTransport->addressToString(),
                      " discarded, application not interested!");
    }
  } else {
