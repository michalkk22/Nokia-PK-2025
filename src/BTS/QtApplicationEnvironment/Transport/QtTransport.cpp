...
QtTransport::QtTransport(common::ILogger &logger, QAbstractSocket *socket)
  ...
  QObject::connect(socket, &QAbstractSocket::readyRead,
                   std::bind(&QtTransport::readMessageFromSocket, this));
  QObject::connect(socket, &QAbstractSocket::disconnected,
                   std::bind(&QtTransport::handleClosingConnection, this));
  QObject::connect(this, SIGNAL(sendMessageSignal(QByteArray)), this,
                   SLOT(sendMessageSlot(QByteArray)));
}
...
void QtTransport::registerMessageCallback(
    ITransport::MessageCallback messageCallback) {
  this->messageCallback = messageCallback;
}

void QtTransport::registerDisconnectedCallback(
    ITransport::DisconnectedCallback disconnectedCallback) {
...
bool QtTransport::sendMessage(BinaryMessage message) {
  ...
  array.append(reinterpret_cast<char *>(message.value.data()),
               message.value.size());
...
void QtTransport::handleClosingConnection() {
  if (disconnectedCallback) {
    ...
    logger.logError("Connection lost from: ", addressToString(),
                    " - application not interested!");
  }
}
...
void QtTransport::readMessageFromSocket() {
  ...
  while ((bytesAvailable = socket->bytesAvailable()) >= sizeSize) {
    ...
    BinaryMessage::SizeType messageLength =
        sizeDecoder.readNumber<BinaryMessage::SizeType>();
    ...
    logger.logDebug("Message received from: ", addressToString(),
                    " body: ", message);

    if (messageCallback) {
      messageCallback(std::move(message));
    } else {
      logger.logError("Message received from: ", addressToString(),
                      " - application not interested");
    }
  }
}
