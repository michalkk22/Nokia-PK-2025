...
void QtSelectionListMode::addSelectionListItemSlot(QString label,
                                                   QString tooltip) {
...
void QtSelectionListMode::clearSelectionListSlot() { listWidget.clear(); }
...
void QtSelectionListMode::constructGui() {
  ...
  listWidget.setStyleSheet("QListWidget::item:selected { border-color: "
                           "darkblue; background: rgba(100, 100, 100, 200);}");
...
void QtSelectionListMode::connectSignals() {
  connect(&listWidget, &QListWidget::doubleClicked,
          [this](const QModelIndex &) { emit itemDoubleClicked(); });
  connect(this, SIGNAL(addSelectionListItemSignal(QString, QString)), this,
          SLOT(addSelectionListItemSlot(QString, QString)));
  connect(this, SIGNAL(clearSelectionListSignal()), this,
          SLOT(clearSelectionListSlot()));
}
...
IUeGui::IListViewMode::OptionalSelection
QtSelectionListMode::getCurrentItemIndex() const {
...
void QtSelectionListMode::addSelectionListItem(const std::string &label,
                                               const std::string &tooltip) {
  emit addSelectionListItemSignal(QString::fromStdString(label),
