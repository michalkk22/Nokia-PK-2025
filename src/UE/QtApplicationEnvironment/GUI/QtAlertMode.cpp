...
QtAlertMode::QtAlertMode(QtPhoneNumberEdit &phoneNumberEdit,
...
void QtAlertMode::connectSignals() {
  QObject::connect(this, SIGNAL(setTextSignal(QString)), this,
                   SLOT(setTextSlot(QString)));
}

void QtAlertMode::activateSlot() { activateWithPhoneNumberEditDisabled(); }

void QtAlertMode::setText(const std::string &text) {
  emit setTextSignal(QString::fromStdString(text));
}

void QtAlertMode::setTextSlot(QString text) { alertText.setText(text); }
