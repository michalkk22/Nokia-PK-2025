...
namespace ue {
    ...
    QtUeGui::QtUeGui(ILogger &logger)
        : logger(logger), mainWindow(), centralWidget(&mainWindow),
          frame(&centralWidget), acceptButton(&centralWidget),
          rejectButton(&centralWidget), homeButton(&centralWidget),
          ...
          alertMode(phoneNumberEdit, stackedWidget), textViewMode(smsComposeMode) {
        initGUI();
        initInternalSignals();
    }
    ...
    void QtUeGui::start() { mainWindow.show(); }
    ...
    void QtUeGui::initGUI() {
        ...
        frame.move(cellPhoneSize.width() * 53 / 412,
                    cellPhoneSize.height() * 90 / 800);
    ...
    void QtUeGui::initInternalSignals() {
        ...
        QObject::connect(&rejectButton, SIGNAL(clicked()), this, 
                        SLOT(onRejectClicked()));
        QObject::connect(&homeButton, SIGNAL(clicked()), this, SLOT(onHomeClicked()));

        QObject::connect(&listViewMode, SIGNAL(itemDoubleClicked()), this, 
                        SLOT(onItemSelected()));
        QObject::connect(&callMode, SIGNAL(textEntered()), this, 
                        SLOT(onTextEntered()));

        QObject::connect(this, SIGNAL(setConnectedStateSignal(QString, bool)), this, 
                        SLOT(setConnectedStateSlot(QString, bool)));
        QObject::connect(this, SIGNAL(setNewMessageSignal(bool)), this, 
                        SLOT(setNewMessageSlot(bool)));
    }
    ...
    void QtUeGui::showNewSms(bool present) { emit setNewMessageSignal(present); }
    ...
    IUeGui::IDialMode &QtUeGui::setDialMode() { return activateMode(dialMode); }

    IUeGui::ICallMode &QtUeGui::setCallMode() { return activateMode(callMode); }

    IUeGui::ITextMode &QtUeGui::setAlertMode() { return activateMode(alertMode); }
