...
ApplicationEnvironment::ApplicationEnvironment(int &argc, char *argv[])
    : configuration(ApplicationEnvironment::readConfiguration(argc, argv)),
      myPhoneNumber(
          PhoneNumber{configuration->getNumber<decltype(PhoneNumber::value)>(
              "phone", 123)}),
      logFile(logFilename(myPhoneNumber)), loggerBase(logFile),
      logger(loggerBase, getPhoneNumberPrefix(myPhoneNumber)),
      qApplication(argc, argv), gui(logger), transport(*configuration, logger) {
}

ue::IUeGui &ApplicationEnvironment::getUeGui() { return gui; }
...
std::int32_t
ApplicationEnvironment::getProperty(std::string const &name,
