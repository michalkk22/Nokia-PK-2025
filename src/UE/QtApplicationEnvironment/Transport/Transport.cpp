...
Transport::Transport(common::MultiLineConfig &configuration,
                     common::ILogger &loggerBase)
  ...
  if (manager.capabilities() &
    ...
    QObject::connect(session.get(), &QNetworkSession::opened,
  ...
  void (QTcpSocket::*errorSignal)(QAbstractSocket::SocketError) =
      &QTcpSocket::error;
  QObject::connect(socket.get(), errorSignal, [this](auto socketError) {
    this->handleError(socketError);
  });
  QObject::connect(socket.get(), &QTcpSocket::readyRead,
                   [this]() { this->readData(); });
  QObject::connect(socket.get(), &QAbstractSocket::disconnected,
                   std::bind(&Transport::handleClosingConnection, this));

  connect(this, SIGNAL(sendMessageSignal(QByteArray)), this,
          SLOT(sendMessageSlot(QByteArray)), Qt::QueuedConnection);
}
