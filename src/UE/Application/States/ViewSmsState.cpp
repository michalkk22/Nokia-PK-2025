...
ViewSmsState::ViewSmsState(Context &context, std::size_t smsIndex)
    : BaseState(context, "ViewSmsState"), viewingSmsIndex(smsIndex) {
...
void ViewSmsState::handleSmsReceived(common::PhoneNumber from,
                                     std::string text) {
  logger.logInfo("Received new SMS from ", from,
                 " while reading another message.");
