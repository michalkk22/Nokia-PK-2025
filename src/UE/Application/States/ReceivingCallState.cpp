...
ReceivingCallState::ReceivingCallState(Context &context, common::PhoneNumber fromNumber) : ConnectedState(context, "ReceivingCallState"), fromNumber(fromNumber) {}

void ReceivingCallState::handleUiAction(std::optional<std::size_t> selectedIndex) {}

void ReceivingCallState::handleUiAccept() {
    logger.logInfo("User pressed accept in ReceivingCall state - trying to accept call.");
