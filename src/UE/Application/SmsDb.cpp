...
namespace ue
{
    ...
    std::size_t SmsDb::addReceivedSms(common::PhoneNumber fromNumber, const std::string& message)
    {
        messages.emplace_back(fromNumber, message);
        return messages.size() - 1;
    }

    std::size_t SmsDb::addSentSms(common::PhoneNumber to, const std::string &message, SmsMessage::Status initialStatus)
    {
