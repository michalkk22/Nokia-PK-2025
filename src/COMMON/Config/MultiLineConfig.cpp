...
std::string MultiLineConfig::getString(const std::string &key,
...
const MultiLineConfig::KeyValueRegex MultiLineConfig::keyValueRegex =
    makeKeyValueRegex();
MultiLineConfig::KeyValueRegex MultiLineConfig::makeKeyValueRegex() {
  ...
  returnValue.lineRegex =
      std::regex("(" + keyRegex + "=" + valueRegex + ")?(#.*)?");
  // str(1) is key=value              ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
void MultiLineConfig::parseLine(const std::string &line) {
  ...
  if (std::regex_search(line, matchKeyValue, keyValueRegex.lineRegex) &&
      matchKeyValue.size() >=
          std::max(keyValueRegex.keyIndex, keyValueRegex.valueIndex)) {
...
void MultiLineConfig::assertNumberLength(std::size_t readSequenceLength,
                                         const std::string &text) const {
  if (readSequenceLength != text.length()) {
    throw std::invalid_argument(
        "There are characters in \"" + text +
        "\" after pos: " + std::to_string(readSequenceLength));
  }
}

void MultiLineConfig::getNumber(
    Details::NumberTypeToRead<true> &valueToRetrieve,
    const std::string &text) const {
  std::size_t pos = 0;
...
void MultiLineConfig::getNumber(
    Details::NumberTypeToRead<false> &valueToRetrieve,
    const std::string &text) const {
  std::size_t pos = 0;
