...
TestCommands::Command TestCommands::readRepeatCommand(std::istream &is) {
  unsigned howMany =
      readArg<unsigned>(is, "'repeat' needs how-many-times number");
...
TestCommands::Command TestCommands::readGroupCommand(std::istream &is) {
  unsigned howMany =
      readArg<unsigned>(is, "'group' needs how-many-commands number");
...
TestCommands::Command TestCommands::readThreadCommand(std::istream &is) {
  ...
  return [subCommand](Parameters parameters) {
    std::thread t([subCommand, parameters] { subCommand(parameters); });
    t.detach();
  };
}
...
TestCommands::Command TestCommands::readWriteCommand(std::istream &is) {
  std::string message =
      readArg<std::string>(is, "`write` needs message to write!");
  return [message](Parameters parameters) { parameters.printText(message); };
}

TestCommands::Command TestCommands::readWaitCommand(std::istream &is) {
  std::uint32_t waitTime =
      readArg<std::uint32_t>(is, "'wait' needs wait time (ms)");
...
TestCommands::Command TestCommands::readSendCommand(std::istream &is) {
  MessageId messageId =
      readArg<MessageId>(is, "'send' needs MessageId", MessageId::Sib);
...
template <typename T>
T TestCommands::readArg(std::istream &is, std::string onFailure,
                        T defaultValue) {
...
std::string TestCommands::readMessageBody(std::istream &is) {
  std::string body =
      readArg<std::string>(is, "Message body missing (for 0-bytes use 0x)");
