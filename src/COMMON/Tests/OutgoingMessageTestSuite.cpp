...
protected:
  ...
  void getMessage() { messageToSend = objectUnderTest.getMessage(); }

  template <typename T> PhoneNumber phone(T value) {
...
TEST_P(OutgoingMessageTestSuite, shallEncodeMessageHeaderAndString) {
  ...
  ASSERT_THAT(text, ElementsAreArray(
                        messageToSend.value.data() + sizeof(MessageHeader),
...
TEST_P(OutgoingMessageTestSuite,
       shallEncodeMessageHeaderAndTwoNumbersAndString) {
  ...
  ASSERT_EQ(sizeof(MessageHeader) + 3u + text.length(),
            messageToSend.value.size());
  ASSERT_EQ(highByte, messageToSend.value[sizeof(MessageHeader)]);
  ASSERT_EQ(lowByte, messageToSend.value[sizeof(MessageHeader) + 1]);
  ...
  ASSERT_THAT(text,
              ElementsAreArray(
                  messageToSend.value.data() + sizeof(MessageHeader) + 3u,
                  messageToSend.value.size() - sizeof(MessageHeader) - 3u));
}

INSTANTIATE_TEST_SUITE_P(
    DifferentHeaders, OutgoingMessageTestSuite,
    Values(MessageHeader{MessageId::AttachRequest, 0x02, 0x03},
