...
protected:
  ...
  const Matcher<std::string> matchNumber2 =
      HasSubstr("1.23"); // std::to_string(float) gives wrong result...
...
TEST_F(ILoggerTestSuite, shallPrintValuesToDebugStream) {
  auto matchPrintout =
      AllOf(HasSubstr(message1), HasSubstr(std::to_string(number1)),
            HasSubstr(message2), matchNumber2);
...
TEST_F(ILoggerTestSuite, shallPrintValuesToErrorStream) {
  auto matchPrintout = AllOf(HasSubstr(message2), matchNumber2);
