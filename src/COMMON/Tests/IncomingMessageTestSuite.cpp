...
protected:
  const MessageHeader messageHeader{MessageId::CallDropped, PhoneNumber{0x12},
                                    PhoneNumber{0x34}};
...
TEST_F(IncomingMessageTestSuite,
       shallAcceptLessThanHeaderButHeaderWontBePresent) {
  Input lessThanHeader{Input::Value(sizeof(MessageHeader) - 1)};
  makeObjectUnderTest(lessThanHeader);
  ASSERT_THROW(assertHeader(), IncomingMessage::ReadEx);
}

IncomingMessageTestSuite::Input IncomingMessageTestSuite::createInputForHeader(
    const MessageHeader &messageHeader) {
  static_assert(sizeof(PhoneNumber::value) == 1,
                "You need to redefine this test");
  static_assert(sizeof(MessageId) == 1, "You need to redefine this test");
  return {{get(messageHeader.messageId), messageHeader.from.value,
           messageHeader.to.value}};
}
...
TEST_F(IncomingMessageTestSuite, shallReadHeaderAndNumber) {
  Input input = createInputForHeader(messageHeader);
  Input::Value::value_type numbers[] = {numberByte1, numberByte2, numberByte3,
                                        numberByte4};
  std::copy(std::begin(numbers), std::end(numbers),
            std::back_inserter(input.value));
...
TEST_F(IncomingMessageTestSuite, shallReadHeaderAndByteNumberString) {
  Input input = createInputForHeader(messageHeader);
  Input::Value::value_type numbers[] = {oneByte, numberByte1, numberByte2,
                                        numberByte3, numberByte4};
  std::copy(std::begin(numbers), std::end(numbers),
            std::back_inserter(input.value));
  std::copy(text.begin(), text.end(), std::back_inserter(input.value));
