...
TEST_F(MultiLineConfigTestSuite,
       shallReadPresentKeyWithoutTrailingWhitespacesAndComment) {
...
TEST_F(MultiLineConfigTestSuite, shallFailToReadSignedAsUnsignedValue) {
  makeObjectUnderTest(" key = -123 ");
  unsigned int value;
  ASSERT_THROW(objectUnderTest->getNumber<unsigned int>("key"),
               std::out_of_range);
}
...
TEST_F(MultiLineConfigTestSuite, shallReadLongMultiLineConfig) {
  ...
  ASSERT_NO_THROW(value1 =
                      objectUnderTest->getNumber<unsigned long long>("key1"));
