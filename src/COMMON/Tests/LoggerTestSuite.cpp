...
protected:
  ...
  LevelDefinition debugLevel = {&ILogger::logDebug<const std::string &>,
                                "[DEBUG]"};
  LevelDefinition infoLevel = {&ILogger::logInfo<const std::string &>, ""};
  LevelDefinition errorLevel = {&ILogger::logError<const std::string &>,
                                "[ERROR]"};
  ...
  LevelDefinition &getParam() { return *levels.at(GetParam()); }
  std::string getLog1() { return getParam().log1.str(); }
  std::string getLog2() { return getParam().log2.str(); }
...
INSTANTIATE_TEST_SUITE_P(LoggerFunctions, LoggerTestSuite,
                         Values(ILogger::DEBUG_LEVEL, ILogger::INFO_LEVEL,
                                ILogger::ERROR_LEVEL));
